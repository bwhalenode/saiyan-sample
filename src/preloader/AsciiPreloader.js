...
// Warm the hero assets before entry; readiness also has a six-second timeout.
const READY_ASSETS = [
  '/images/hero-1.webp',
  '/images/hero-1.jpg',
...
export class AsciiPreloader {
  ...
  _trackReady() {
    const assetLoads = READY_ASSETS.map(
      (src) =>
        new Promise((res) => {
          const img = new Image()
  ...
  _spawnGlitchChars(p) {
    ...
    const mob = IS_MOB()
    const count = mob ? 4 + Math.floor(Math.random() * 4) : 8 + Math.floor(Math.random() * 8)
  ...
  _maybeShowPrompt() {
    ...
    if (this._loaded) {
      // Asset loading has settled or reached its timeout; enable entry.
      this._armed = true
