...
export function initTimeline(lenis) {
  ...
  document
    .querySelectorAll(
      '.nav__links a[href^="#"], .nav__socials a[href^="#"], .nav__brand[href^="#"]',
    )
    .forEach((link) => {
  ...
  if (inflectionTitle && !inflectionTitle.dataset.split) {
    inflectionTitle.innerHTML = inflectionTitle.textContent
      .trim()
      .split(/\s+/)
